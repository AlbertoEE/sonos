...
### Paso 3 — Ejecutar el flujo crear-* correspondiente

1. Lee el archivo de la idea elegida para obtener el tipo y el texto.
2. Según el tipo, lee la plantilla de prompt correspondiente:

   | type | plantilla de prompt |
   |------|---------------------|
   | `boss` | `/home/k8s/dev/sonos/.pi/prompts/crear-boss.md` |
   | `escena` | `/home/k8s/dev/sonos/.pi/prompts/crear-escena.md` |
   | `viaje` | `/home/k8s/dev/sonos/.pi/prompts/crear-viaje.md` |
   | `set-armadura` | `/home/k8s/dev/sonos/.pi/prompts/crear-set-armadura.md` |
   | `set-habilidades` | `/home/k8s/dev/sonos/.pi/prompts/crear-set-habilidades.md` |

3. Ejecuta ese flujo **usando el texto de la idea como `$ARGUMENTS`**. Sigue sus instrucciones exactamente igual que si el usuario hubiera invocado directamente la plantilla `crear-*` con ese texto.
